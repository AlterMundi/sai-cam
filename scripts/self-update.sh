...
#
# IMPORTANT: The entire script body is wrapped in main() so that Bash reads
# it fully into memory before execution. This prevents a self-modification
# bug: install.sh overwrites this file on disk during updates, and without
# the wrapper, Bash would read stale/shifted bytes mid-execution (status=127).
#
# Usage:
